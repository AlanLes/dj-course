    ...
    if sessions:
        ...
        for session in sessions:
            if session.get('error'):
                ...
                title_str = f"Tytuł: {session['title']}" if session['title'] else "Brak tytułu"
                console.print_help(f"- ID: {session['id']} ({title_str}, Wiadomości: {session['messages_count']}, Ost. aktywność: {session['last_activity']})")
        console.print_help("------------------------------------")
    else:
