...
export const RouteSummary: React.FC<RouteSummaryProps> = ({
  ...
  const pointTypeBadgeColors: Record<string, string> = {
    pickup: 'bg-green-500',
  ...
  const pointTypeLabelColors: Record<string, string> = {
                ...
                {route.estimatedCompletion && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-blue-600">
                      {formatDateTime(route.estimatedCompletion)}
                    </span>
                  </div>
                )}
              </div>
              ...
              {['pickup', 'delivery', 'rest', 'fuel', 'border'].map((type) => {
                ...
                return (
                  <div key={type} className="flex justify-between">
                    ...
                    <span className={`font-medium ${pointTypeColors[type]}`}>{count}</span>
                  </div>
                );
              })}
            </div>
                ...
                {route.points.map((point, index) => (
                  ...
                  >
                    <div className="flex-shrink-0">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-medium ${pointTypeBadgeColors[point.type] ?? 'bg-gray-500'}`}
                      >
                        {index + 1}
                      </div>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span
                          className={`px-2 py-1 text-xs rounded-full font-medium ${pointTypeLabelColors[point.type] ?? 'bg-gray-100 text-gray-800'}`}
                        >
