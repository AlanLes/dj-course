...
export const RouteControls: React.FC<RouteControlsProps> = ({
  ...
  const requiresRestStops = (): boolean => {
    const requiredRestStops = Math.floor(
      Math.max(route.estimatedDuration / 270, route.totalDistance / 360)
    );
    return requiredRestStops > route.points.filter((p) => p.type === 'rest').length;
  };

  const getRestStopWarning = (): string | null => {
    const restStops = route.points.filter((p) => p.type === 'rest').length;
    const requiredRestStops = Math.max(
        ...
        {!showAddOptions ? (
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setShowAddOptions(false)} className="text-gray-400 hover:text-gray-600">
            ...
            <div className="grid grid-cols-1 gap-2">
              {pointTypes.map(({ type, label, icon, color }) => (
                <button
                  key={type}
                  onClick={() => { onAddPoint(type); setShowAddOptions(false); }}
        ...
        {route.points.length > 0 && requiresRestStops() && (
          <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
            <div className="flex items-start gap-2">
              ...
              <div>
                <h4 className="font-semibold text-red-800 text-sm mb-1">Mandatory Rest Stops Required</h4>
                <p className="text-red-700 text-xs">{getRestStopWarning()}</p>
                <button
