...
export const DraggableRoutePoint: React.FC<DraggableRoutePointProps> = ({
  ...
  const getTypeBadgeColor = (type: RoutePoint['type']): string => {
    const colors: Record<RoutePoint['type'], string> = {
  ...
  return (
    ...
    >
      ...
      <div className="flex-shrink-0">
        <div
          className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-medium ${getTypeColor(point.type)}`}
        >
          {index + 1}
        </div>
      </div>
